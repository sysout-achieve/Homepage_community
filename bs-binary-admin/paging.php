...
//hw paging 시작.
	...
	if($allPost > 8){
		...
		//첫 페이지가 아니라면 처음 버튼을 생성
		if($page != 1) {
			$paging .= '<li class="page page_start"><a href="./bod_free_view.php?bno='.$bNo.'&page=1&open=1">처음</a></li>';
		}
		//첫 섹션이 아니라면 이전 버튼을 생성
		if($currentSection != 1) {
			$paging .= '<li class="page page_prev"><a href="./bod_free_view.php?bno='.$bNo.'&page=' . $prevPage . '&open=1">이전</a></li>';
		}
		for($i = $firstPage; $i <= $lastPage; $i++) {
			if($i == $page) {
				$paging .= '<li class="page current">' . $i . '</li>';
			} else {
				$paging .= '<li class="page"><a href="./bod_free_view.php?bno='.$bNo.'&page=' . $i . '&open=1">' . $i . '</a></li>';
			}
		}
		//마지막 섹션이 아니라면 다음 버튼을 생성
		if($currentSection != $allSection) {
			$paging .= '<li class="page page_next"><a href="./bod_free_view.php?bno='.$bNo.'&page=' . $nextPage . '&open=1">다음</a></li>';
		}
		//마지막 페이지가 아니라면 끝 버튼을 생성
		if($page != $allPage) {
			$paging .= '<li class="page page_end"><a href="./bod_free_view.php?bno='.$bNo.'&page=' . $allPage . '&open=1">끝</a></li>';
		}
		$paging .= '</ul>';
	}
		/* 페이징 끝 */
		$currentLimit = ($onePage * $page) - $onePage; //몇 번째의 글부터 가져오는지
		$sqlLimit = ' limit ' . $currentLimit . ', ' . $onePage; //limit sql 구문
		$sql = 'select * from comment_free where co_no=co_order and b_no=' .$bNo.' order by co_date desc' . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
	}//comment paging 끝.
?>
